...
SELECT * FROM Staff WHERE Department = "経理部";
...
SELECT s.* FROM Shop s JOIN Area a ON s.AreaCode = a.AreaCode WHERE a.AreaName = '関東';
...
SELECT s.ShopCode, s.ShopName, a.AreaName FROM Shop s JOIN Area a ON s.AreaCode = a.AreaCode;
-- 8. 「新宿」店の在庫数が10以上の商品の商品コード（GoodsCode）、商品名（GoodsName）、在庫数（quantity）を抽出して下さい。
SELECT s.GoodsCode, g.GoodsName, s.Quantity FROM Stocks s 
JOIN Goods g ON s.GoodsCode = g.GoodsCode
JOIN Shop shop ON s.ShopCode = shop.ShopCode
WHERE shop.ShopName = '新宿' AND s.Quantity >= 10;